...
class ConnectorToDm:
    ...
    данных в контексте схемы 'dm'.

    Для взаимодействия с БД используется менеджер контекстов, например:
        with ConnectorToDm(db_conn_params) as conn
    ...
    Методы:
        get_raw_data() -> dict:
            Метод для получения данных из таблиц схемы 'dm'.
        load_transformed_data(transformed_data: dict) -> None:
            Метод для загрузки данных в схему 'dm'.
    """
    ...
    def __init__(self, db_conn_params: str) -> None:
        ...
        self.__temp_schema: str = 'temp_dm'  # Имя временной схемы БД "temp_dm"
    ...
    def __enter__(self) -> 'ConnectorToDm':
        """
        Метод для входа в контекст класса 'ConnectorToDm'.

        Returns
        Экземпляр класса 'ConnectorToDm'
        """
    ...
    def __exit__(self, exc_type, exc_value, traceback) -> None:
        """
        Метод для выхода из контекста класса 'ConnectorToDm'.
    ...
    def get_raw_data(self) -> dict:
        """
        Метод для получения данных из схемы 'dm'.
        """
        ...
        with Session() as session:
            with session.begin():
                # Формируем SELECT SQL-запросы для каждой таблицы в списке __tables
    ...
    def load_transformed_data(self, transformed_data: dict) -> None:
        """
        Метод для загрузки преобразованных данных в схему 'dm'.
    ...
    def __switch_schems(self) -> None:
        """
        Метод для переключения схем 'dm' и 'temp_dm' в БД.
        """
